...
var HTLN = {
    ...
    //
    ///**
    // * Sets a loading gif on the element specified
    // */
    ...
    //
    ///**
    // * Removes specified loading state
    // */
...
HTLN.maps = {
    ...
    _searchBoxId: 'search-location',
    _myLocation: 'pan-to-location',
    _saveStationClass: 'save-station',
    _detailsPopupSelector: 'hotel-details-popup',
    _detailsPopupCloseSelector: 'hotel-details-popup__close',
    _defaultZoom: 7,
    _geolocate: true,
    _defaultLat: 45.95115,
    ...
    init: function() {
        ...
        this._infoWindow = new google.maps.InfoWindow({maxWidth: 360});
        this.initMarkers();
        this.initDetailsBox();
        this.initPopupClose();
    ...
    initPopupClose: function() {
        var self = this;
        $('.' + self._detailsPopupCloseSelector).on('click', function() {
            $(this).closest('.' + self._detailsPopupSelector).hide();
            $('html').css({overflowY: 'visible'});
        });
    },

    /**
     * Performs client geolocation and sets the map center to the current location of the client (if client accepts it)
     */
    geolocate: function(zoom) {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(function (position) {
                ...
                self._map.panTo(initialLocation);
                zoom ? self._map.setZoom(zoom) : self._map.setZoom(12);
    ...
    //
    //        self._searchMarker.setVisible(false);
    //
    //        if (!place.geometry) {
    //            return;
    //        }
    ...
    //        }
    //        self._searchMarker.setIcon({
    //            url: 'resources/img/marker-my-location.png'
    //        });
    //        self._searchMarker.setPosition(place.geometry.location);
    //        self._searchMarker.setVisible(true);
    //
    //    });
    //},
     ...
     */
    myLocationEvent: function() {
        var self = this;
        $('#' + this._myLocation).on('click', function() {
            self.geolocate();
            //self._searchMarker.setVisible(false);
        });
    },
     ...
     * Handle markers
     */
    initMarkers: function(filteredStations) {
        var self = this;
        this.hotels = {};
        ...
        if(filteredStations) {
            ...
            $.when(this.getHotels()).done(function(){
                ...
                self.initAddMarkers(hotels);
            });
        }
    },

    /**
     * Initializes addition of markers on the map
     * @param stations - array of stations to be added as markers on the map
     */
    initAddMarkers: function(hotels) {
    ...
    /**
     * Effectively adds markers to map
     * @param data - array of json objects which act as data to be added to the map as markers
     * @param markerImgPath - path to marker image
     */
    addMarkers: function(data, markerImgPath) {
        var markerImage = new google.maps.MarkerImage(markerImgPath, new google.maps.Size(31, 46));
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            var fn;
            if(data[i].id) {
                fn = this.markerClickFunction(data[i], marker);
                this._markers.push(marker);
            } else {
                fn = this.nearbyPlaceMarkerClickFunction(data[i], marker);
                this._placesMarkers.push(marker);
            }
     ...
     * @returns {Function}
     */
    markerClickFunction: function(hotel, marker) {
        ...
        return function(e) {
            ...
            //places
            var placesUrl = "";
            if(hotel.nearbyPlacesUrl) {
                placesUrl = '<span class="h-tooltip-places">nearby places ></span>';
            }

            var content = '' +
                '<div class="h-tooltip">' +
                    '<div class="h-tooltip-address">'+hotel.address+'</div>' +
                    '<hr>' +
                    '<div class="h-tooltip-hotel-name">'+hotel.name+'</div>' +
                    '<div class="h-tooltip-hotel-details">></div>' +
                    '<div class="h-tooltip-hotel-description">'+hotel.shortDescription+'</div>' +
                    stars +
                    placesUrl +
                '</div>';

            HTLN.maps._infoWindow.setContent(content);
            HTLN.maps._infoWindow.open(HTLN.maps._map, marker);

            //click event for hotel details
            var $tooltipDetails = $('.h-tooltip-hotel-details');
            $tooltipDetails.off();
            var fn_details = HTLN.maps.tooltipLinkEvent(hotel);
            $tooltipDetails.on('click', fn_details);

            //click event for nearby places
            var $places = $('.h-tooltip-places');
            $places.off();
            var fn_places = HTLN.maps.nearbyPlacesEvent(hotel, marker);
            $places.on('click', fn_places);
        };
    },

    removePlacesMarkers: function () {
        for(var i in this._placesMarkers) {
            this._placesMarkers[i].setMap(null);
        }

        this._placesMarkers = [];
    },

    nearbyPlaceMarkerClickFunction: function (place, marker) {
        return function(e) {
            e.cancelBubble = true;
            e.returnValue = false;
            if (e.stopPropagation) {
            ...
            //compute place extras
            var extra = "";
            if(place.specifications) {
                extra += '<div class="h-tooltip-place-extras">';
                extra += '<p>Extra information</p>';
                extra += '<p>'+place.specifications+'</p>';
                extra += '</div>';
            }

            var content = '' +
                '<div class="h-tooltip">' +
                '<div class="h-tooltip-address">'+place.address+'</div>' +
                '<hr>' +
                '<div class="h-tooltip-place-name">'+place.name+'</div>' +
                '<a href="'+place.detailsUrl+'" class="h-tooltip-place-details">></a>' +
                '<img class="h-tooltip-place-image" src="'+place.photo+'">' +
                '<div class="h-tooltip-place-description">'+place.description+'</div>' +
                '<hr>' +
                '<div class="h-tooltip-directions"><p>Directions</p><p>'+place.guidance+'</p></div>' +
                '<hr>' +
                extra +
                '</div>';
    ...
    tooltipLinkEvent: function(hotel) {
        var self = this,
            $box = $('#' + self._elementId).parent().find('.station-details');

        this._activeHotel = hotel;
    ...
    nearbyPlacesEvent: function(hotel) {
        var self = this;

        return function() {
            HTLN.maps._infoWindow.close();

            $.getJSON(hotel.nearbyPlacesUrl, function(result) {
                self.addMarkers(result, self._nearbyPlacesMarker);
            });
        };
    }
};
